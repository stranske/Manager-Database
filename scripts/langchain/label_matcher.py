...
try:
    semantic_matcher_mod = import_module("scripts.langchain.semantic_matcher")
except ModuleNotFoundError:
    semantic_matcher_mod = import_module("semantic_matcher")

semantic_matcher = semantic_matcher_mod
...
def build_label_vector_store(
    ...
    *,
    ...
    resolved = client_info or semantic_matcher_mod.get_embedding_client(model=model)
    if resolved is None:
...
def find_similar_labels(
    ...
    *,
    ...
    if hasattr(store, "similarity_search_with_relevance_scores"):
        ...
        fallback_matches = _keyword_matches(label_store.labels, query, threshold=threshold)
        fallback_matches.sort(key=lambda match: match.score, reverse=True)
