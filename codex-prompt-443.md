...
**Progress:** 17/38 tasks complete, 21 remaining

### Scope
PR #432 (addressing issue #429) implemented `asyncio.CancelledError` handling and 24-hour memory stabilization tests. This follow-up adds the remaining feature: a configurable sampling interval to reduce performance impact.
...
- [x] Add `interval_s` parameter to `start_background_profiler()` in `api/memory_profiler.py`
- [x] Define scope for: Expose `interval_s` parameter in `start_background_profiler()`. (verify: confirm completion in repo)
- [x] Implement focused slice for: Expose `interval_s` parameter in `start_background_profiler()`. (verify: confirm completion in repo)
- [x] Validate focused slice for: Expose `interval_s` parameter in `start_background_profiler()`. (verify: confirm completion in repo)
- [x] Define scope for: Ensure `_run_profiler_loop` correctly handles the `interval_s` parameter. (verify: confirm completion in repo)
- [x] Implement focused slice for: Ensure `_run_profiler_loop` correctly handles the `interval_s` parameter. (verify: confirm completion in repo)
- [x] Validate focused slice for: Ensure `_run_profiler_loop` correctly handles the `interval_s` parameter. (verify: confirm completion in repo)
- [x] Document the new parameter in the function docstring
- [x] Add a unit test verifying the interval parameter is respected
- [x] Define scope for: Write a unit test to verify the default behavior when `interval_s` is not provided.
- [x] Implement focused slice for: Write a unit test to verify the default behavior when `interval_s` is not provided.
- [x] Validate focused slice for: Write a unit test to verify the default behavior when `interval_s` is not provided.
- [x] Define scope for: Write a unit test to verify the profiler loop respects a custom `interval_s` value.
- [x] Implement focused slice for: Write a unit test to verify the profiler loop respects a custom `interval_s` value.
- [x] Validate focused slice for: Write a unit test to verify the profiler loop respects a custom `interval_s` value.
- [x] Add `interval_s` parameter to `start_background_profiler()` in `api/memory_profiler.py`
- [x] Define scope for: Expose `interval_s` parameter in `start_background_profiler()`. (verify: confirm completion in repo)
- [x] Implement focused slice for: Expose `interval_s` parameter in `start_background_profiler()`. (verify: confirm completion in repo)
- [x] Validate focused slice for: Expose `interval_s` parameter in `start_background_profiler()`. (verify: confirm completion in repo)
...
- [x] `start_background_profiler()` accepts an optional `interval_s` parameter (default: current behavior)
- [x] The profiler loop uses the provided interval between snapshots
- [x] A unit test confirms the interval parameter works correctly
...
- Add `interval_s` parameter to `start_background_profiler()` in `api/memory_profiler.py`
- Define scope for: Expose `interval_s` parameter in `start_background_profiler()`. (verify: confirm completion in repo)

### Suggested Next Task
- Define scope for: Ensure `_run_profiler_loop` correctly handles the `interval_s` parameter. (verify: confirm completion in repo)
