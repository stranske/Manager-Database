...
### Scope
The application lacks a proper health check endpoint for monitoring and alerting. Operations teams need visibility into system health to respond to outages quickly.
...
- [ ] Add a metrics endpoint at /metrics for Prometheus scraping.
- [ ] Configure alerting thresholds (response time > 500ms = warning).
- [ ] Export Grafana dashboard JSON to docs/grafana/.
- [ ] Document runbook for common health check failures in docs/runbooks/health-checks.md.
...
- [ ] Implement retry logic with exponential backoff for flaky checks
- [ ] Add metrics endpoint at /metrics for Prometheus scraping
- [ ] Configure alerting thresholds (response time > 500ms = warning)
- [ ] Set up health check dashboard in Grafana (template)
- [ ] Document runbook for common health check failures

### Acceptance Criteria
The PR is complete when ALL of these are satisfied:

- [x] /health returns 200 OK when all services up.
- [ ] /health returns 503 Service Unavailable with details when any check fails.
- [x] /health/detailed returns JSON with each component status.
- [ ] /metrics exposes health_check_duration_seconds histogram.
- [ ] Response time < 200ms for basic health check.
- [ ] Circuit breaker opens after 3 consecutive failures.
- [ ] Retry logic uses exponential backoff (100ms, 200ms, 400ms).
- [ ] Grafana dashboard JSON exported to docs/grafana/.
- [ ] Runbook added to docs/runbooks/health-checks.md.
- [x] /health returns 200 OK when all services up
- [ ] /health returns 503 Service Unavailable with details when any check fails
- [x] /health/detailed returns JSON with each component status
- [ ] /metrics exposes health_check_duration_seconds histogram
