...
**Progress:** 35/39 tasks complete, 4 remaining
...
### Scope
The PDF spec describes "Universal search: one query returns filings, holdings tables, news, and user notes (ranked)." The current \`ui/search.py\` (83 lines) searches news and notes in separate tabs using different queries. There is no unified search that spans all entity types and returns a single ranked result set with snippets.

### Tasks
Complete these in order. Mark checkbox done ONLY after implementation is verified:

- [x] Create a search service module \`api/search.py\` (or extend existing \`ui/search.py\`) with a unified search function:
  ```python
  def universal_search(query: str, conn, limit: int = 20) -> list[SearchResult]:
      """Search across all entity types and return ranked results."""
  ```
- [x] Define \`SearchResult\` model:
  ```python
  class SearchResult(BaseModel):
      entity_type: str          # 'filing', 'holding', 'news', 'document', 'manager'
      entity_id: int
      manager_name: str | None
      headline: str             # Title or summary line
      snippet: str              # Relevant text excerpt
      relevance: float          # 0.0-1.0 ranking score
      url: str | None           # Link to source
      timestamp: str | None     # When the entity was created/published
  ```
- [x] Implement FTS search for each entity type (Postgres):
  - [x] **Managers**: Search name, aliases (using \`array_to_string\` or GIN index)
  - [x] **Filings**: Search type + raw_key + period_end context
  - [x] **Holdings**: Search name_of_issuer, cusip (exact match for CUSIP)
  - [x] **News**: Search headline + body_snippet using \`to_tsvector/plainto_tsquery\`
  - [x] **Documents**: Search text using FTS + optional vector similarity
- [x] Implement FTS search for SQLite fallback:
  - [x] Use LIKE queries for basic text matching
  - [x] Use the existing \`search_documents()\` from \`embeddings.py\` for vector search
- [x] Merge and rank results:
  - [x] Assign base relevance score by entity type (news > filings > documents > holdings)
  - [x] Boost score by FTS rank (ts_rank for Postgres)
  - [x] For vector search results, convert distance to relevance (1 - normalized_distance)
  - [x] Sort by combined relevance score descending
  - [x] Apply limit
- [x] Add a FastAPI endpoint \`GET /api/search?q=<query>&limit=20\`:
  - [x] Returns \`list[SearchResult]\`
  - [x] Supports optional \`entity_type\` filter parameter
- [x] Update \`ui/search.py\` to call the unified search:
  - [x] Single search box at the top
...
- [x] Create \`tests/test_search.py\`:
  ...
  - [x] Verify relevance ranking (higher-ranked results first)
  - [x] Verify entity_type filter works
  - [x] Verify empty query returns empty results

### Acceptance Criteria
The PR is complete when ALL of these are satisfied:

- [x] A single search query (e.g., "Elliott") returns results from managers, filings, news, and documents
- [x] Results are ranked by relevance with entity type badges
