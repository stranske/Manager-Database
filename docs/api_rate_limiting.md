...
Clients that call `POST /api/chat` or `POST /api/chat/feedback` should throttle
requests locally by session and use conservative backoff after a 429 response.
...
- `api/chat.py` applies that limiter in `POST /api/chat`.
