...
| `llm/cost_tracking.py` | dialect-aware | SQLite table setup avoids audited autoincrement tokens; Postgres keeps `bigserial` setup and backend placeholders. | Follow-up #1010 resolved: `python scripts/check_dialect_portability.py --no-allowlist llm/cost_tracking.py scripts/seed_universe.py scripts/resolve_aliases.py` now passes. | Allowed by gate without an allowlist entry. |
| `scripts/seed_universe.py` | dialect-aware | SQLite manager setup avoids audited autoincrement tokens and uses table-valued column introspection; Postgres uses `ALTER TABLE ... IF NOT EXISTS` and information schema. | Follow-up #1010 resolved: `python scripts/check_dialect_portability.py --no-allowlist llm/cost_tracking.py scripts/seed_universe.py scripts/resolve_aliases.py` now passes. | Allowed by gate without an allowlist entry. |
| `scripts/resolve_aliases.py` | dialect-aware | SQLite manager id detection uses table-valued column introspection; Postgres uses information schema and backend placeholders for alias updates. | Follow-up #1010 resolved: `python scripts/check_dialect_portability.py --no-allowlist llm/cost_tracking.py scripts/seed_universe.py scripts/resolve_aliases.py` now passes. | Allowed by gate without an allowlist entry. |
