...
| `Manager-Intel-Platform.md:98` | EDGAR access should respect fair-use by throttling requests. | implemented-and-verified | `adapters/edgar.py:_request_with_retry` applies an env-configurable steady-state outbound request-rate governor before all EDGAR calls; `tests/test_edgar_integration.py::test_outbound_request_rate_governor` verifies pacing sleep and the disabled interval path. | #1185 |
